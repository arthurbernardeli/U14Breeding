...
server <- function(input, output, session) {
  ...
  observeEvent({ list(catalog_reactive(), idc_reactive()) }, {
    # Apply STAGE filter to both datasets before extracting unique CODE values
    catalog <- catalog_reactive() %>% filter(STAGE == "advanced")
    idc <- idc_reactive() %>% filter(STAGE == "advanced")
  ...
  filtered_input_data <- reactive({
    ...
    catalog_reactive() %>% filter(CODE == input$trial, STAGE == "advanced") %>% droplevels()
  })
  
  # ✅ Reactive expression for filtered IDC data
  filtered_idc_data <- reactive({
    req(input$idcfile, input$trial)
    idc_reactive() %>% filter(CODE == input$trial, STAGE == "advanced") %>% droplevels()
  })
  ...
  processed_data <- eventReactive(input$process, {
    ...
    summary <- summary %>% select(CODE, STRAIN, MD, MD_DAP, LG, HT, PRO, OIL, SumPO, MEAL_PRO, MealProductYield, COY, EPVOil, Fibre)
    ...
    merged_final <- merged_final %>% select(CODE, STRAIN, YIELD_MIN, YIELD_AVG, YIELD_MAX,MD, MD_DAP, LG, HT, PRO, OIL, SumPO, MEAL_PRO, MealProductYield, COY, EPVOil, Fibre) 
    ...
    merged_adjusted_final <- merged_adjusted_final %>% select(CODE, STRAIN, Estimate, StdErr, YIELD_AVG, YIELD_MIN, YIELD_MAX,MD, MD_DAP, LG, HT, PRO, OIL, SumPO, MEAL_PRO, MealProductYield, COY, EPVOil,PROLbAc, Fibre)
    ###
    ...
    merged_rep <- merged_rep %>% select(CODE, STRAIN, Estimate, StdErr, MD, MD_DAP, REPS, LG, HT, YIELD_AVG, YIELD_MIN, YIELD_MAX, YPD, PRO, OIL, SumPO, MEAL_PRO, MealProductYield, COY, EPVOil, PROLbAc, Fibre)
    ...
    save <- save %>%
      select(STRAIN, INFO, CODE, PREV_CODE, Estimate, StdErr, MD_DAP, MD, REPS, LG, HT, YIELD_AVG, YIELD_MIN, YIELD_MAX, YPD, 
